...
import { deleteAdmin } from "../../redux/adminsSlice";

const DeleteAdminPopup = ({ close, title, item, id }) => {
  const dispatch = useDispatch();
  const { deletingAdmin } = useSelector((state) => state.admins);
        ...
        <div className="p-6 flex flex-col gap-4 h-auto sm:max-h-96 overflow-auto scrollbar-hide">
          ...
          <div className="flex flex-row gap-4">
            <button
              ...
              className="bg-primary hover:bg-opacity-80 p-2 rounded-lg text-white w-full flex flex-row items-center justify-center"
            >
              {deletingAdmin ? <Spinner /> : "Yes"}
            </button>
            <button
              onClick={close}
              disabled={deleteAdmin}
